  ...
  if (!advancedMode) {
    return (
      <Card>
        ...
        <p className="text-sm text-content-2">{t("Advanced Mode is off. Enable it from")}<b>{t("Settings → Advanced Mode")}</b>{t("to open the in-panel Config Studio.")}</p>
      </Card>
    );
  }
  ...
  return (
    <div className="space-y-4 animate-fade-up">
      <div className="flex flex-wrap items-center gap-2">
        ...
        <Select value={mode} onChange={(e) => setMode(e.target.value as EditMode)} className="w-44" aria-label={t("edit mode")}>
          <option value="tree">{t("visual tree (default)")}</option>
          <option value="raw">{t("raw document — pro")}</option>
          <option value="ops">{t("patch operations — pro")}</option>
        </Select>
        <Button variant="secondary" size="sm" onClick={() => run((o) => doPreview.mutate(o))} loading={doPreview.isPending} disabled={!effectiveCore}>
          <Eye size={13} />{t("validate & diff")}</Button>
      ...
      <div className="grid gap-4 xl:grid-cols-2">
          ...
          {mode === "tree" ? (() => {
            ...
            return parseError ? (
              <div className="p-2 text-[12px] text-danger">
                <p className="mb-2 font-medium">{t("the document isn't valid JSON right now — fix it in raw mode:")}</p>
            ...
            <div className="space-y-2.5 p-1.5">
              <p className="text-[11px] text-content-3">{t("RFC-6902 patch ops against the current document — precise, validated, and diffed before apply.")}</p>
              {ops.map((op, i) => (
                <div key={i} className="grid gap-2 rounded-xl border border-border p-2.5 sm:grid-cols-[110px_1fr_1fr_36px]">
                  ...
                  <Textarea rows={1} value={typeof op.value === "string" ? op.value : JSON.stringify(op.value ?? null)} dir="ltr" placeholder={t("value (JSON)")}
                  ...
                  <Button variant="ghost" size="icon" aria-label={t("remove op")} onClick={() => setOps(ops.filter((_, xi) => xi !== i))}><Trash2 size={13} /></Button>
                </div>
              ))}
        ...
        <Card className="p-3">
          <div className="flex items-center gap-2 px-1.5 pb-2 text-[11px] text-content-3">
            {preview ? (
              preview.valid
                ? <><CheckCircle2 size={13} className="text-ok" />{t("schema-valid — diff of the proposed change")}</>
